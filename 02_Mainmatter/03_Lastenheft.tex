\section{Lastenheft}
\label{Lastenheft}
In diesem Kapitel wird erklärt, wie das Lastenheft aufgebaut ist, wie die Lastfälle bestimmt wurden und welche Annahmen dabei getroffen werden. Da das Lastenheft ist auf der Seite \pageref{Lastenheft A} zu finden.\\
...
\end{center}
  ...
  \begin{description}
    ...
    In einem ersten Ansatz, die erfahrene Beschleunigung beim Überfahren einer Bresmmschwelle zu bestimmen, wird der Solar Butterfly als ein Einmassenschwinger-System modelliert und die Beschleunigung beim Überfahren einer Sinusförmigen Bremsschwelle numerisch ermittelt. Der Rechnungsweg wird im Anhang \ref{Berechnung 1.1} ausführlich erläutert. Die Exceltabelle, mit welcher die Berechnung durchgeführt wurde, ist im elektronischen Anhang ANHANG zu finden.
    ...
    Zu diesem Ergebnis muss gesagt werden, dass davon ausgegangen werden kann, dass die erhaltene Beschleunigung zu hoch liegt. So wurde zum Beispiel die Effekte der Reifen und die rotatorische Trägheit des Solar Butterflys nicht berücksichtigt, was Abminderungen der berechneten Beschleunigung zur folge hätten.\\
  ...
  Im Lastenheft werden vier unterschiedliche Neigungslasten definiert. Sie unterscheiden sich in ihrer Richtung der Neigung.
...
Der Modus \emph{C} beschreibt den Solar Buttefly im ausgefahren Zustand. Alle Panelen, Seitenmodule und Stützen ausgefahren. Personen und das Mobiliar können frei im Solar Butterfly verteilt sein.\\
...
Als \emph{Personenlast} werden Lasten verstanden, welche durch Personen inneren des Solar Butteflys verursacht werden. Im Hauptkörper, sowie in den ausziehbaren Seitenmodulen sollen sich maximal 6 Personen befinden. Im Kopf, sowie im Heck des Solar Butterfly, hat es platzbedingt nur Raum für maximal drei Personen. Das durchschnittliche Gewicht einer Person wird auf 80 kg geschätzt.\\
Die Personenlasten \emph{1.1} bis \emph{1.6} ergeben sich aus der Masse von 6 Personen à 80 kg. Es wird jeweils vereinfacht angenommen, dass sich alle Personen an einem Punkt befinden, wie dies in der Abbildung \ref{Boden} dargestellt ist. Diese Lasten werden daher als Vektorlasten im FEM-Modell eingeleitet. Die Personenlasten \emph{1.7} und \emph{1.8} werden als Streckenlasten eingeleitet.
...
\paragraph{Mobiliar}\mbox{}\\
Insegsammt werden drei Lastfälle für die Belastung durch Mobiliar definiert (Für jedes Raumelement einen). Die Belastungen durch das Mobiliar werden jeweils als Flächenlast im FEM-Modell eingeleitet.\\
...
\begin{landscape}% Landscape page
  ...
  \begin{tabularx}{\linewidth}{llcXccc}
    ...
    1.5 & Rotatorische Beschleunigung      & 4.4 $\frac{rad}{s^2}$ & Rotatorische Beschleunigung um den Vektor der Fahrtrichtung & 0.3 & 60 & 18\\
